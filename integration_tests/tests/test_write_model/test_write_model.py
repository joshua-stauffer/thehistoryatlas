...
def publish_new_citation_broker():
    ...
    broker = TestBroker(
        queue_name="test_broker",
        listen_to="event.emitted"
    )
    return broker
...
def add_new_citation_events(publish_new_citation_broker):
    events = [from_dict(event_dict) for event_dict in publish_new_citation_broker.results[0]]
    return events
