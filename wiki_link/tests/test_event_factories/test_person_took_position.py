...
def test_create_wiki_event_location_in_position_name(mock_query):
    mock_query.get_label.side_effect = lambda id, language: (
        "Test Location" if id == "Q789" else "Position in Test Location"
    )
    ...
    # Location should not be repeated in summary
    assert "Position in Test Location" in event.summary
    assert not any(
        part.strip().startswith("at Test Location") for part in event.summary.split(",")
    )

    # Place tag should use position name but location ID
    assert event.place_tag.name == "Position in Test Location"
    assert event.place_tag.wiki_id == "Q789"
