...
class PersonDied(EventFactory):
    ...
    def entity_has_event(self) -> bool:
        has_event = (
            PLACE_OF_DEATH in self.entity.claims and DATE_OF_DEATH in self.entity.claims
        )
    ...
    def create_wiki_event(self) -> list[WikiEvent]:
        ...
        place_name = self.query.get_label(id=self._place_of_death_id(), language="en")
        geo_location = self.query.get_geo_location(id=self._place_of_death_id())
    ...
    def _place_of_death_id(self) -> str:
        return self.entity.claims[PLACE_OF_DEATH][0]["mainsnak"]["datavalue"]["value"][
            "id"
        ]
    ...
    def _time_definition(self):
        return build_time_definition_from_claim(
            time_claim=next(
                claim
                for claim in self.entity.claims[DATE_OF_DEATH]
