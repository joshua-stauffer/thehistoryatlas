...
from the_history_atlas.apps.readmodel.schema import Base, Tag, Person, Place, Time, Name, Source, Story, StoryName, Summary, TagInstance, Citation, StorySummary, TagNameAssoc
    ...
    def create_seed_data():
        ...
        def create_name_variations_and_associate(base_name, variations, tag_id):
            for name in variations:
                ...
                tag_name_associations.append(TagNameAssoc(tag_id=tag_id, name_id=name_obj.id))
        ...
        for i in range(300):
            ...
            if tag_type == "PERSON":
                tags.append(
                    Person(id=tag_id, names=[
                        Name(id=random_uuid(), name=name)
                        for name in [f"Person {i+1}", f"P{i+1}", f"Individual {i+1}"]
                    ]),
                )

            elif tag_type == "PLACE":
                tags.append(
                    Place(id=tag_id, names=[
                        Name(id=random_uuid(), name=name)
                ...
                translations = {"en": f"The Year {time_value}", "it": f"L'Anno {time_value}", "de": f"Das Jahr {time_value}", "fr": f"L'Année {time_value}", "es": f"El Año {time_value}"}
                tags.append(
                    Time(id=tag_id, time=time_value, precision=precision, names=[
        ...
        for i in range(100):
            ...
            source_published_date = fake.date_this_century()  # Generate a realistic date
        ...
        for tag in tags:
            ...
            for lang, name in translations.items():
                session.add(StoryName(id=random_uuid(), story_id=story_id, name=name, lang=lang))
        ...
        for i in range(1000):
            ...
            summary_text = fake.paragraph(nb_sentences=5)  # Generate realistic summary text
            summary = Summary(id=summary_id, text=summary_text)
            session.add(summary)

            for tag in [person_tag, place_tag, time_tag]:
                tag_name = fake.word()  # Placeholder for tag-specific text to simulate realism
                start_char = summary_text.find(tag_name)
                stop_char = start_char + len(tag_name)
                session.add(TagInstance(id=random_uuid(), tag_id=tag.id, summary_id=summary_id, start_char=start_char, stop_char=stop_char))

            source = random.choice(sources)
            session.add(Citation(id=random_uuid(), summary_id=summary_id, source_id=source.id))

            for story in session.query(Story).filter(Story.tag_id.in_([person_tag.id, place_tag.id, time_tag.id])):
                session.add(StorySummary(id=random_uuid(), story_id=story.id, summary_id=summary_id, order=i))
