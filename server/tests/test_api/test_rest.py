...
class TestUpdateNullStoryOrder:
    def test_update_null_story_order(self, client, auth_headers):
        # Use environment variable for DB connection
        db_uri = os.environ.get("THA_DB_URI")
        engine = create_engine(db_uri)

        # Create a repository with the engine
        repository = Repository(engine, source_trie=Trie())
        ...
        with engine.connect() as connection:
            ...
            # Create summaries
            summary_ids = [uuid.uuid4() for _ in range(3)]
            connection.execute(
                text(
                    """
                INSERT INTO summaries (id, text)
                ...
                {
                    "summary_id1": str(summary_ids[0]),
                    "summary_id2": str(summary_ids[1]),
                    "summary_id3": str(summary_ids[2]),
                },
            )

            # Create time tags for each summary
            time_ids = [uuid.uuid4() for _ in range(3)]
            connection.execute(
                text(
                    """
                INSERT INTO tags (id, type)
                ...
                {
                    "time_id1": str(time_ids[0]),
                    "time_id2": str(time_ids[1]),
                    "time_id3": str(time_ids[2]),
                },
            )
            ...
            connection.execute(
                ...
                VALUES 
                    (:time_id1, '1980-01-01', 'gregorian', 9),
                    (:time_id2, '1990-01-01', 'gregorian', 9),
                    (:time_id3, '1985-01-01', 'gregorian', 9)
                """
                ),
                {
                    "time_id1": str(time_ids[0]),
            ...
            # Create tag instances linking summaries to their time tags
            connection.execute(
                text(
                    """
                INSERT INTO tag_instances (id, tag_id, summary_id, story_order)
                VALUES 
                    (:id1, :time_id1, :summary_id1, 0),
                    (:id2, :time_id2, :summary_id2, 0),
                    (:id3, :time_id3, :summary_id3, 0)
                """
                ),
                {
                    "id1": str(uuid.uuid4()),
            ...
            # Create tag instances with null story_order values
            connection.execute(
                text(
                    """
                INSERT INTO tag_instances (id, tag_id, summary_id, story_order, after)
                VALUES 
                    (:id1, :tag_id, :summary_id1, NULL, '[]'::jsonb),
                    (:id2, :tag_id, :summary_id2, NULL, '[]'::jsonb),
                    (:id3, :tag_id, :summary_id3, NULL, '[]'::jsonb)
                """
                ),
                {
                    "tag_id": str(tag_id),
        ...
        repository.update_null_story_order(tag_id)
        ...
        # Check that story_order values are now set and in correct order
        assert len(rows) == 3
        assert all(
            row[1] is not None for row in rows
        ), "All story_order values should be non-null"
        ...
        with engine.connect() as connection:
            ...
            connection.execute(
                ...
                WHERE tag_id = :tag_id 
                OR tag_id = ANY(ARRAY[:time_ids]::uuid[])
                """
                ),
                {"tag_id": str(tag_id), "time_ids": [str(tid) for tid in time_ids]},
            )
            ...
            connection.execute(
                text(
                    """
                DELETE FROM times 
                WHERE id = ANY(ARRAY[:time_ids]::uuid[])
                """
                ),
                {"time_ids": [str(tid) for tid in time_ids]},
            )
            ...
            connection.execute(
                ...
                WHERE id = ANY(ARRAY[:summary_ids]::uuid[])
                """
                ),
                {"summary_ids": [str(sid) for sid in summary_ids]},
            )
            connection.commit()

    def test_update_null_story_order_with_after(self, client, auth_headers):
        # Use environment variable for DB connection
        db_uri = os.environ.get("THA_DB_URI")
        engine = create_engine(db_uri)
        ...
        # Create a new tag and summaries for testing
        tag_id = uuid.uuid4()
        summary_ids = [uuid.uuid4() for _ in range(3)]
        time_ids = [uuid.uuid4() for _ in range(3)]

        # Setup test data with 'after' relationships
        with engine.connect() as connection:
            # First create the tag
            connection.execute(
                text(
            ...
            # Create tag instances with null story_order values and 'after' relationship
            # Event 3 should come after Event 1 despite its date
            connection.execute(
                text(
                    """
                INSERT INTO tag_instances (id, tag_id, summary_id, story_order, after)
                VALUES 
                    (:id1, :tag_id, :summary_id1, NULL, '[]'::jsonb),
                    (:id2, :tag_id, :summary_id2, NULL, '[]'::jsonb),
                    (:id3, :tag_id, :summary_id3, NULL, jsonb_build_array(:summary_id1))
                """
                ),
                {
                    "tag_id": str(tag_id),
        ...
        # Check that story_order values are now set and respect the 'after' relationship
        assert len(rows) == 3
        assert all(row[1] is not None for row in rows)

        # Find the positions of summary_id1 and summary_id3
        positions = {str(row[0]): i for i, row in enumerate(rows)}

        # Verify that summary_id3 comes after summary_id1
        assert positions[str(summary_ids[2])] > positions[str(summary_ids[0])]
    ...
    def test_update_null_story_order_sparse_integers(self, client, auth_headers):
        # Use environment variable for DB connection
        db_uri = os.environ.get("THA_DB_URI")
        engine = create_engine(db_uri)
        ...
        # Create a new tag for testing
        tag_id = uuid.uuid4()
        summary_ids = [uuid.uuid4() for _ in range(4)]
        time_ids = [uuid.uuid4() for _ in range(4)]
        ...
        with engine.connect() as connection:
            ...
            connection.execute(
                ...
                VALUES 
                    ...
                    (:time_id2, '1990-01-01', 'gregorian', 9),
                    (:time_id3, '1985-01-01', 'gregorian', 9),
                    (:time_id4, '1995-01-01', 'gregorian', 9)
                """
                ),
                {
                    "time_id1": str(time_ids[0]),
            ...
            # Create tag instances with some existing and some null story_order values
            connection.execute(
                text(
                    """
                INSERT INTO tag_instances (id, tag_id, summary_id, story_order, after)
                VALUES
                    (:id1, :tag_id, :summary_id1, 1000, '[]'::jsonb),
                    (:id2, :tag_id, :summary_id2, 3000, '[]'::jsonb),
                    (:id3, :tag_id, :summary_id3, NULL, '[]'::jsonb),
                    (:id4, :tag_id, :summary_id4, NULL, '[]'::jsonb)
                """
                ),
                {
                    "tag_id": str(tag_id),
        ...
        # Check that story_order values are now set and maintain sparse integers
        assert len(rows) == 4
        assert all(row[1] is not None for row in rows)

        # Verify that the existing story_order values (1000, 3000) are preserved
        story_orders = [row[1] for row in rows]
        assert 1000 in story_orders
        assert 3000 in story_orders

        # Verify that new story_order values maintain ordering
        assert story_orders == sorted(story_orders)
