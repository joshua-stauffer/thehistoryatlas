...
class BrokerBase:
    ...
    def __init__(
        ...
        self._connection_settings = {
            ...
            "SAFE_URL": f"amqp://{broker_username}" + f":******@{network_host_name}/",
            "VHOST": "/",
            "SSL": False,
        ...
        # Channel settings
        self._channel_settings = {"PUBLISHER_CONFIRMS": False}
        # Exchange settings
    ...
    async def add_message_handler(
        self, routing_key: str, callback: Callable, timeout=None
    ) -> None:
        ...
        await self.__queue.bind(
            self.__exchange, routing_key=routing_key, timeout=timeout
        )
    ...
    def create_message(
        ...
        reply_to: str = None,
        expiration: Union[
            int, datetime.datetime, float, datetime.timedelta, None
        ] = None,
        timestamp: Union[
            int, datetime.datetime, float, datetime.timedelta, None
        ] = None,
    ...
    async def connect(
        self, retry=True, retry_timeout=0.5, max_attempts: Union[int, None] = None
    ):
        ...
        try:
            ...
            log.error(
                f"PyBroker is unable to connect to AMQP broker "
            ...
            if retry:
                ...
                return await self.connect(
                    retry=retry, retry_timeout=retry_timeout, max_attempts=max_attempts
                )
            else:
