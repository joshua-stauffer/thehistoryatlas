...
def test_make_CITATION_ADDED(composer, basic_meta, citation_args):
    ...
    e = composer.events[0]
    assert e == {**basic_meta, "type": "CITATION_ADDED", "payload": citation_args}
...
def test_make_PERSON_ADDED(composer, basic_meta, person_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "PERSON_ADDED", "payload": person_args}
...
def test_make_PLACE_ADDED(composer, basic_meta, place_args_with_coords):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "PLACE_ADDED", "payload": place_args_with_coords}
...
def test_make_TIME_ADDED(composer, basic_meta, time_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "TIME_ADDED", "payload": time_args}
...
def test_make_PERSON_TAGGED(composer, basic_meta, person_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "PERSON_TAGGED", "payload": person_args}
...
def test_make_PLACE_TAGGED(composer, basic_meta, place_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "PLACE_TAGGED", "payload": place_args}
...
def test_make_TIME_TAGGED(composer, basic_meta, time_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "TIME_TAGGED", "payload": time_args}
...
def test_make_META_ADDED(composer, basic_meta, meta_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "META_ADDED", "payload": meta_args}
...
def test_make_SUMMARY_ADDED(composer, basic_meta, summary_added_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "SUMMARY_ADDED", "payload": summary_added_args}
...
def test_make_SUMMARY_TAGGED(composer, basic_meta, summary_tagged_args):
    ...
    assert len(composer.events) == 1
    e = composer.events[0]
    assert e == {**basic_meta, "type": "SUMMARY_TAGGED", "payload": summary_tagged_args}


def test_composer_maintains_order(
    composer, person_args, place_args, time_args, meta_args
):
    assert len(composer.events) == 0
    composer.make_PERSON_ADDED(**person_args)
    composer.make_PLACE_TAGGED(**place_args)
...
def test_composer_orders_summary_first(
    composer,
    person_args,
    place_args,
    time_args,
