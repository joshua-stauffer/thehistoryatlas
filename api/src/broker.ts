...
export class Broker {
  ...
  private publishRPC(
    ...
    if (
      ...
      throw new Error(
        'Stream is full. Try again after receiving "drain" event.'
      );
  ...
  private async handleRPCCallback(
  ...
  private declareExchanges = (channel: Amqp.Channel) => {
    ...
    for (let exchConf of this.exchanges) {
      channel
        ...
        .catch((err) => {
          console.log(
            `Caught error ${err} while declaring exchange ${exchConf.name}.`
          );
        });
    }
  };
  ...
  private createQueue = (channel: Amqp.Channel, exchConf: ExchangeDetails) => {
    ...
    channel
      ...
      .catch((err) => {
        console.log(
          `Got error ${err} while creating queue ${exchConf.queueName}`
        );
      });
  };
  ...
  private bindQueue = (channel: Amqp.Channel, exchConf: ExchangeDetails) => {
    ...
    channel
      ...
      .catch((err) => {
        console.log(
          `Got error ${err} while binding queue ${queueName} to ${name}`
        );
      });
  };
  ...
  private decode(msg: Buffer): any {
    try {
      ...
      console.error(
        `Broker.decode was likely passed a poorly formed message: ${err}`
      );
      return null;
    }
  }
