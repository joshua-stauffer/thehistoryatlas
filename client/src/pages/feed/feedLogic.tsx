...
import {
  ...
  GetSummariesByGUIDResult,
  GetSummariesByGUIDVars,
...
export const useFeedLogic = () => {
  ...
  const [currentSummaries, setCurrentSummaries] = useState<
    GetSummariesByGUIDResult["GetSummariesByGUID"]
  >([]);
  const [currentCoords, setCurrentCoords] = useState<MarkerData[]>([]);
  const [focusedGeoEntities, setFocusedGeoEntities] = useState<
    FocusedGeoEntity[]
  >([]);
  ...
  if (!currentEntity)
    throw Error(
      "Default Entity must be loaded before the feed can be rendered."
    );
  ...
  const {
    ...
    error: manifestError,
    data: manifestData,
  } = useQuery<GetManifestResult, GetManifestVars>(GET_MANIFEST, {
    variables: {
      GUID: currentEntity.entity.guid,
      entityType: currentEntity.entity.type,
    },
  });
  //  -- load summaries for slice of manifest currently in event feed
  const { loading: summariesLoading, data: summariesData } = useQuery<
    GetSummariesByGUIDResult,
    GetSummariesByGUIDVars
  >(GET_SUMMARIES_BY_GUID, { variables: { summary_guids: currentEvents } });
  ...
  if (currentSummaries.length) {
    const focusedSummary = currentSummaries.find(
      (summary) => summary.guid === currentFocus.focusedGUID
    );
    if (focusedSummary) {
      // take the first time tag for now
      const timeTag = focusedSummary.tags.find(
        (tag) => tag.tag_type === "TIME"
      ) as TimeTag;
      currentDate = timeTag && timeTag.name ? timeTag.name : null;
    }

    const summaryList =
      !manifestLoading && !manifestError ? currentSummaries : [];
  }
