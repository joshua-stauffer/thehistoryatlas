...
export const SourceContent = (props: SourceContentProps) => {
  ...
  const guid = isOpen && citationGUIDs.length 
    ? citationGUIDs[0] : ''
  const {
    data,
    loading, 
    ...
    GET_CITATION_BY_GUID,
    { variables: { citationGUID: guid } }
  )
  console.log({ data, loading, error })
  if (error) return (
    <CardContent>
      <Typography variant="h4">Oops!</Typography>
      <Typography variant="body2">There was an error loading this citation. Please try again later</Typography>
  </CardContent>
  )
  if (loading || !data?.GetCitationByGUID.meta) return (
    <Skeleton 
      sx={{
        height: 100
      }}
