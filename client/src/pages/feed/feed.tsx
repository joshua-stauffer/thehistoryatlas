      ...
      <Grid container spacing={10} direction={"row"} justifyItems={"center"}>
        ...
        >
          {currentSummaries.map((summary, i) => (
            <Paper sx={{ marginTop: "55px" }}>
              <Grid item id={`event-card-${i}`} xs={12} md={12}>
                <Paper>
                  <FeedCard summary={summary} currentFocus={currentFocus} />
                </Paper>
              </Grid>
              <Grid item xs={12} md={12} id={`event-tags-${i}`}>
                <TagBox
                  tags={summary.tags}
        ...
        <Hidden smDown>
          ...
          <Grid item md={6}>
            <SingleEntityMap
              coords={
                focusedGeoEntities.map((entity) => entity.coords) ??
                getCoordsFromCurrentCoords(currentCoords)
              }
              mapTyle={"natGeoWorld"}
              size={"MD"}
              title={currentFocus.focusedGUID}
