...
export const TagEntities = (props: TagEntitiesProps) => {
  ...
  const { loading, error, data } = useQuery<
    TextAnalysisResult,
    TextAnalysisVars
  >(GET_TEXT_ANALYSIS, { variables: { text: text } });
  console.log({ data });
    ...
    setTags(
      boundaries.map((boundary) => {
        if (
          ...
          boundary.start_char === discoveredEntities[0].start_char
        ) {
          const ent = discoveredEntities.shift();
          if (!ent || !ent.guids.length)
            return {
          ...
          return {
            ...
            name: ent.text,
            guid: ent.guids[0],
            type: people.includes(ent) ? "PERSON" : "TIME",
          };
        }
        return {
          start_char: boundary.start_char,
      ...
      <Grid container spacing={2} direction="column" alignContent="center">
        ...
        <Grid item xs={12} sm={6}>
          <Paper>
            {currentEntity ? (
              ...
              <>
                <Typography align="center">
                  Click a word to tag a Person, Place, or Time
                </Typography>
                <Button variant="contained" onClick={addNewTag}>
                  Tag Entity without reference
                </Button>
              </>
            )}
          </Paper>
        ...
        <Grid item xs={12}>
          <Button
            variant="contained"
            onClick={() =>
              tagEntities(tags.filter((tag) => tag.type !== "NONE"))
            }
