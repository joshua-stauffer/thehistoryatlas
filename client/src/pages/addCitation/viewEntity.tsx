...
export const ViewEntity = (props: ViewEntityProps) => {
  ...
  const {
    ...
    GET_ENTITY_SUMMARIES_BY_GUID,
    { variables: { guids: [tag.guid ?? ''] } }
  )

  // the query only returns something useful if there's a guid
  if (!tag.guid || !data?.GetEntitySummariesByGUID.length) return (
    <>
    <Typography variant="h3">{tag.name}</Typography>
    <Typography variant="body1">This {tag.type.toLowerCase()} doesn't exist yet in our database.</Typography>
    <Typography variant="body1">You'll be the first to tag {tag.type === "PERSON" ? "them" : "it"}!</Typography>
    </>
  )
  ...
  return (
    <>
      ...
      <Typography variant="body1">Earliest citation: {entity.first_citation_date}</Typography>
      <Typography variant="body1">Latest citation: {entity.last_citation_date}</Typography>
      <Typography variant="body1"> Citation count: {entity.citation_count}</Typography>
