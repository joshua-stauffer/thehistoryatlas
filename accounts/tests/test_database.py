...
def test_admin_can_add_user(db, other_user_details, active_admin_token):
    token, user = db.add_user(token=active_admin_token, user_data=other_user_details)
...
def test_non_admin_cant_add_user(loaded_db, user_details, active_token):
    ...
    with pytest.raises(UnauthorizedUserError):
        loaded_db.add_user(token=active_token, user_data=user_details)
...
def test_partial_update_user(loaded_db, user_details, active_token):
    ...
    loaded_db.update_user(token=active_token, user_data=updated_user_info)
    ...
    for field in PROTECTED_FIELDS:
        with pytest.raises(UnauthorizedUserError):
            loaded_db.update_user(
                token=active_token, user_data={field: "cant change this"}
            )
...
def test_login(loaded_db, admin_user_details):
    ...
    token = loaded_db.login(username=username, password=password)
...
def test_deactivate_account_with_admin_token(
    ...
    token, user_details = loaded_db.deactivate_account(
        token=active_admin_token, user_id=user_id
    )
    assert token is active_admin_token
...
def test_deactivate_account_with_non_admin_token(
    ...
    with pytest.raises(UnauthorizedUserError):
        loaded_db.deactivate_account(token=active_token, user_id=user_id)
