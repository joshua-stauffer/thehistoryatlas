...
def test_broker_base_init(
    base, username, password, network_host_name, exchange_name, queue_name
):
    ...
    # connection
    assert (
        base._connection_settings.get("URL")
...
async def test_connect(
    monkeypatch, event_loop, base, username, password, network_host_name
):

    store = dict()

    def fake_connect_robust(
        url, virtualhost, timeout, ssl, ssl_options, loop=event_loop
    ):
        store["url"] = url
