...
def event():
    return {
        ...
        "payload": json.dumps({"use your imagination": ", kid!"}),
    }
...
def test_parse_msg_with_str(history):
    res = history._parse_msg({"payload": {"last_event_id": "11"}})
    assert isinstance(res, int)
    assert res == 11


def test_parse_msg_with_int(history):
    res = history._parse_msg({"last_event_id": 27})
...
async def test_handle_request_from_halfway(history, mock_send_tuple):
    send_func, close_func, queue = mock_send_tuple
    await history.handle_request(
        {"type": "REQUEST_HISTORY_REPLAY", "payload": {"last_event_id": 500}},
        send_func,
        close_func,
    )
