...
def db(result_a, name_a, result_b, name_b, result_c, name_c):
    class DB:
        ...
        def get_coords_by_name_batch(names):
            return {name_a: result_a, name_b: result_b, name_c: result_c}
...
@pytest.fixture
def result_a():
    return [{"latitude": 1.1, "longitude": 2.2}, {"latitude": 3.3, "longitude": 4.4}]


@pytest.fixture
def result_b():
    return [{"latitude": 5.5, "longitude": 6.6}, {"latitude": 7.7, "longitude": 8.8}]
...
def coords_by_name_query(name_a):
    return {"type": "GET_COORDS_BY_NAME", "payload": {"name": name_a}}
...
def test_handle_get_coords_by_name_batch(
    qh, coords_by_name_batch_query, result_a, name_a, result_b, name_b, result_c, name_c
):
